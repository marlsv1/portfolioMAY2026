...
const panelDetailBody    = document.querySelector('#panel-project-detail .panel-body');
...
function closeAllPanels() {
  [panelProjects, panelAbout, panelProjectDetail].forEach(p => p.classList.remove('visible'));
...
document.addEventListener('keydown', (e) => {
  if (e.key === 'Escape') {
    if (panelProjectDetail.classList.contains('visible')) {
      ...
      renderBreadcrumb();
    } else if (panelProjects.classList.contains('visible') || panelAbout.classList.contains('visible')) {
      stopAllMedia();
      closeAllPanels();
      navStack = [{ label: 'home', view: 'home' }];
      renderBreadcrumb();
...
const pointerTargets = 'a, button, [role="button"], .folder-btn, .project-item, .panel-close, .tab';
...
document.addEventListener('mousemove', (e) => {
  ...
  cursor.classList.toggle('is-pointer', !!(target && target.closest(pointerTargets)));

  if (!rafId) rafId = requestAnimationFrame(animateEye);
});

document.addEventListener('mousedown', () => { cursor.style.opacity = '0.6'; });
...
let wordIndex = 0, charIndex = 0, isDeleting = false, greetingEl = null;

function typeGreeting() {
  if (!greetingEl) greetingEl = document.getElementById('greeting-word');
  if (!greetingEl) { setTimeout(typeGreeting, 100); return; }
  ...
  if (!isDeleting && charIndex === word.length) { speed = 8000; isDeleting = true; }
...
if (document.readyState === 'loading') {
  document.addEventListener('DOMContentLoaded', () => setTimeout(typeGreeting, 500));
} else {
